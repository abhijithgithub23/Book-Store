...
// Update these methods in src/app/services/api.service.ts

  getBooksByGenre(genre: string, page: number = 1): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/books?genre=${genre}&page=${page}`).pipe(
      map(response => ({
        ...response,
        items: response.items.map((book: any) => this.mapBook(book)) // Map the internal items array
      }))
    );
  }

  searchBooks(query: string, page: number = 1): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/books?q=${query}&page=${page}`).pipe(
      map(response => ({
        ...response,
        items: response.items.map((book: any) => this.mapBook(book)) // Map the internal items array
      }))
